...
def pol(st, P_pick):
    ...
    ax0.text(ENZonsetP-200*delta+0.05, 0.8, str(station), va="center", fontsize=20)
    ax0.text(ENZonsetP-200*delta+0.05, 0.6, 'SNR: '+str(snrZ), va="center", fontsize=20)
    
    ax0.set_xlim(ENZonsetP-200*delta, ENZonsetP+200*delta)
    ax0.set_ylim(-1, 1)
    ...
    if snrZ >= SNRthreshold :
        ...
        try:
            ...
            ax0.set_xlim(ENZonsetP-50*delta, ENZonsetP+50*delta)
            ax0.set_ylim(-1, 1)
            
            ax0.text(ENZonsetP-50*delta+0.02, 0.8, str(station), va="center", fontsize=20)
            ax0.text(ENZonsetP-50*delta+0.02, 0.6, 'SNR: '+str(snrZ), va="center", fontsize=20)
            '''   
...
if __name__ == '__main__':
    ...
    # Create a CSV file for storing station polarity information
    resultcsv = './'
    outFile = str(resultcsv) + 'StationinformationPhasenet.csv'
    
    with open(outFile, mode='w', newline='') as fp:
        writer = csv.writer(fp, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(['StName', 'p_POL(Z)'])
